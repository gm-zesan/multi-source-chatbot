    ...
    <div class="card table-card mb-4">
        <div class="card-header table-header">
            <div class="title-with-breadcrumb">
                <div class="table-title">Lexicon & Dynamic Vocabulary</div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('faqs.index') }}">Knowledge Base</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Lexicons</li>
                    </ol>
                </nav>
            </div>
            ...
            <div class="d-flex align-items-center gap-2 flex-wrap">
                ...
                @if($isSuperAdmin && $availableWorkspaces->isNotEmpty())
                    <form method="GET" action="{{ route('lexicons.index') }}" class="d-flex align-items-center gap-2">
                        <label for="workspace_select" class="small text-muted mb-0 fw-medium">Scope:</label>
                        <select name="workspace_id" id="workspace_select" class="form-select form-select-sm" onchange="this.form.submit()" style="min-width: 170px;">
                ...
                {{-- AI Engine Status --}}
                <span class="badge-subtle {{ ($engineHealth['ok'] ?? false) ? 'badge-subtle-success' : 'badge-subtle-danger' }}">
                    <i class="ri-pulse-line me-1"></i>
                    Engine: {{ ($engineHealth['ok'] ?? false) ? 'Active' : 'Offline' }}
                </span>

                {{-- Hot Reload Sync Button --}}
                <form method="POST" action="{{ route('lexicons.sync') }}" class="d-inline">
                    @csrf
                    ...
                    <button type="submit" class="btn btn-sm btn-outline-primary d-inline-flex align-items-center gap-1">
                        <i class="ri-refresh-line"></i> Sync to Engine
                    </button>
                </form>
            </div>
        </div>
        ...
        {{-- Tab Navigation --}}
        <div class="card-body pb-0 pt-2 border-top">
            <ul class="nav nav-tabs" id="lexiconTabs" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="concepts-tab" data-bs-toggle="tab" data-bs-target="#concepts-pane" type="button" role="tab">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="discovery-tab" data-bs-toggle="tab" data-bs-target="#discovery-pane" type="button" role="tab">
                        <i class="ri-search-eye-line me-1"></i> Unmatched Queries
                        <span class="count-pill ms-1">{{ $unansweredQuestions->count() + $lowConfidenceSearches->count() }}</span>
                    </button>
                </li>
            </ul>
        </div>
                ...
                <div class="tab-pane fade show active" id="domain-pane" role="tabpanel">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <div>
                            ...
                            <small class="text-muted">Exact phrase mapping automatically expanded into search queries.</small>
                        </div>
                        <button class="btn btn-sm btn-primary d-inline-flex align-items-center gap-1" data-bs-toggle="modal" data-bs-target="#addDomainEntryModal">
                            ...
                            <tbody>
                                @foreach($domainEntries as $entry)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge-subtle {{ $entry->status === 'ACTIVE' ? 'badge-subtle-success' : 'badge-subtle-secondary' }}">
                                                {{ $entry->status }}
                                            </span>
                                        </td>
                                        <td class="text-end">
                                            <form action="{{ route('lexicons.domain-entries.destroy', $entry->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Remove this synonym entry?');">
                                                @csrf
                                                @method('DELETE')
                ...
                <div class="tab-pane fade" id="concepts-pane" role="tabpanel">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <div>
                            ...
                            <small class="text-muted">Phrases that trigger canonical concepts or prevent false positive matches.</small>
                        </div>
                        <button class="btn btn-sm btn-primary d-inline-flex align-items-center gap-1" data-bs-toggle="modal" data-bs-target="#addConceptPatternModal">
                            ...
                            <tbody>
                                @foreach($rawConceptPatterns as $p)
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge-subtle {{ $p->status === 'ACTIVE' ? 'badge-subtle-success' : 'badge-subtle-secondary' }}">
                                                {{ $p->status }}
                                            </span>
                                        </td>
                                        <td class="text-end">
                                            <form action="{{ route('lexicons.concept-patterns.destroy', $p->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this pattern?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-outline-danger p-1" title="Delete">
                                                    <i class="ri-delete-bin-line"></i>
                ...
                <div class="tab-pane fade" id="actions-pane" role="tabpanel">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <div>
                            ...
                            <small class="text-muted">Tie-breaking signals to elevate specific action responses over general queries.</small>
                        </div>
                        <button class="btn btn-sm btn-primary d-inline-flex align-items-center gap-1" data-bs-toggle="modal" data-bs-target="#addActionMappingModal">
                            ...
                            <tbody>
                                @foreach($actionMappings as $action)
                                    <tr>
                                        ...
                                        <td><span class="text-success"><i class="ri-arrow-up-line me-1"></i>{{ $action->target_phrase }}</span></td>
                                        <td>
                                            @if($action->penalty_phrase)
                                                <span class="text-danger"><i class="ri-arrow-down-line me-1"></i>{{ $action->penalty_phrase }}</span>
                                            @else
                                                <span class="text-muted">—</span>
                                            @endif
                                        </td>
                                        <td><span class="text-muted small">v{{ $action->version }}</span></td>
                                        <td class="text-end">
                                            <form action="{{ route('lexicons.action-mappings.destroy', $action->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this action mapping?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-outline-danger p-1" title="Delete">
                                                    <i class="ri-delete-bin-line"></i>
                ...
                <div class="tab-pane fade" id="policies-pane" role="tabpanel">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <div>
                            ...
                            <small class="text-muted">Boost specific document types when policy intent keywords are matched.</small>
                        </div>
                        <button class="btn btn-sm btn-primary d-inline-flex align-items-center gap-1" data-bs-toggle="modal" data-bs-target="#addPolicyMappingModal">
                            ...
                            <tbody>
                                @foreach($policyMappings as $pol)
                                    <tr>
                                        ...
                                        <td><span class="badge-subtle badge-subtle-secondary">{{ $pol->target_doc_type }}</span></td>
                                        <td><span class="text-muted small">v{{ $pol->version }}</span></td>
                                        <td>
                                            <span class="badge-subtle {{ $pol->status === 'ACTIVE' ? 'badge-subtle-success' : 'badge-subtle-secondary' }}">
                                                {{ $pol->status }}
                                            </span>
                                        </td>
                                        <td class="text-end">
                                            <form action="{{ route('lexicons.policy-mappings.destroy', $pol->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this policy mapping?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-outline-danger p-1" title="Delete">
                                                    <i class="ri-delete-bin-line"></i>
                ...
                <div class="tab-pane fade" id="discovery-pane" role="tabpanel">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <div>
                            ...
                            <small class="text-muted">Review queries that scored below threshold to identify missing synonyms.</small>
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                                        ...
                                        <tbody>
                                            @forelse($unansweredQuestions as $uq)
                                                <tr>
                                                    ...
                                                    <td><span class="badge-subtle badge-subtle-primary">{{ $uq->occurrence_count }}</span></td>
                                                    <td class="text-end">
                                            ...
                                            @empty
                                                <tr><td colspan="3" class="text-center text-muted py-3 small">No unanswered questions recorded.</td></tr>
                                            @endforelse
                                        </tbody>
                                    </table>
                                </div>
                        ...
                        <div class="col-lg-6">
                            <div class="border rounded p-3 bg-white h-100">
                                <h6 class="fw-bold small text-muted text-uppercase mb-3">Low-Confidence Queries (&lt; 0.60)</h6>
                                <div class="table-responsive">
                                    <table class="table table-sm table-hover mb-0">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            @forelse($lowConfidenceSearches as $log)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span class="badge-subtle {{ $log->final_score > 0.40 ? 'badge-subtle-warning' : 'badge-subtle-danger' }}">
                                            ...
                                            @empty
                                                <tr><td colspan="3" class="text-center text-muted py-3 small">No low-confidence queries recorded.</td></tr>
                                            @endforelse
                                        </tbody>
                                    </table>
                                </div>
        ...
        <form action="{{ route('lexicons.domain-entries.store') }}" method="POST" class="modal-content">
            ...
            <div class="modal-body">
                <div class="mb-3">
                    ...
                    <input type="text" name="concept_key" id="modal_concept_key" class="form-control form-control-sm" placeholder="e.g. PAYMENT_ISSUE, RETURN_POLICY" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="pattern" id="modal_pattern" class="form-control form-control-sm" placeholder="Phrase written by user" required>
                </div>
                <div class="mb-3">
                    <label class="form-label small fw-medium">Expansion Terms <span class="text-danger">*</span></label>
                    <input type="text" name="expansion" id="modal_expansion" class="form-control form-control-sm" placeholder="Terms appended into search query" required>
                </div>
                <div class="mb-3">
    ...
    <div class="modal-dialog">
        <form action="{{ route('lexicons.concept-patterns.store') }}" method="POST" class="modal-content">
            ...
            <div class="modal-body">
                <div class="mb-3">
                    <label class="form-label small fw-medium">Concept Key <span class="text-danger">*</span></label>
                    <input type="text" name="concept_key" class="form-control form-control-sm" placeholder="e.g. RETURN_POLICY, ORDER_TRACKING" required>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3" id="group_concept_phrase">
                    ...
                    <input type="text" name="phrase" class="form-control form-control-sm" placeholder="Trigger or blocking phrase">
                </div>
                <div class="mb-3" id="group_concept_target_doc">
                    <label class="form-label small fw-medium">Target Document Type</label>
                    <input type="text" name="target_doc_type" class="form-control form-control-sm" placeholder="e.g. policy_return, faq">
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
        ...
        <form action="{{ route('lexicons.action-mappings.store') }}" method="POST" class="modal-content">
            ...
            <div class="modal-body">
                <div class="mb-3">
                    ...
                    <input type="text" name="intent_name" class="form-control form-control-sm" placeholder="e.g. invoice, order_track" required>
                </div>
                <div class="mb-3">
                    <label class="form-label small fw-medium">Action Keyword <span class="text-danger">*</span></label>
                    <input type="text" name="action_keyword" class="form-control form-control-sm" placeholder="e.g. download, track" required>
                </div>
                <div class="mb-3">
                    <label class="form-label small fw-medium">Boosted Target Phrase <span class="text-danger">*</span></label>
                    <input type="text" name="target_phrase" class="form-control form-control-sm" placeholder="Target FAQ title to boost" required>
                </div>
                <div class="mb-3">
                    <label class="form-label small fw-medium">Demoted Penalty Phrase</label>
                    <input type="text" name="penalty_phrase" class="form-control form-control-sm" placeholder="Competing FAQ title to demote">
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
        ...
        <form action="{{ route('lexicons.policy-mappings.store') }}" method="POST" class="modal-content">
            ...
            <div class="modal-body">
                <div class="mb-3">
                    ...
                    <input type="text" name="policy_name" class="form-control form-control-sm" placeholder="e.g. return_policy" required>
                </div>
                <div class="mb-3">
                    <label class="form-label small fw-medium">Cue Phrase <span class="text-danger">*</span></label>
                    <input type="text" name="cue_phrase" class="form-control form-control-sm" placeholder="Phrase triggering policy boost" required>
                </div>
                <div class="mb-3">
                    <label class="form-label small fw-medium">Target Document Type <span class="text-danger">*</span></label>
                    <input type="text" name="target_doc_type" class="form-control form-control-sm" placeholder="e.g. policy_return, policy_warranty" required>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
